<template>
    <view class="wrap">
        ...
        <!-- 提交按钮 -->
        <view class="submitNow" v-if="artList.length > 0" @tap="submitNow">编辑说说</view>
    </view>
</template>
<script>
...
export default {
	...
	methods:{
        submitNow : function(){
            ...
            if(this.artList.length < 1){uni.showToast({title:'请填写说说内容', icon:"none"}); return ;}
            ...
            for(var i = 0; i < this.artList.length; i++){
                if(this.artList[i].type == 'image'){
                    if(this.artList[i].content.indexOf('77855.uicp.net') == -1){
...
<!-- <template>
	<view class="wrap">
		...
		<!-- 提交按钮 -->
		<view class="submitNow" v-if="artList.length>0" @tap="submitNow">发布说说</view>
	</view>
</template>
<script>
var _self, loginRes;
